	...
	<div class="container px-6 py-6">
		...
		<div class="flex flex-col items-center justify-between md:flex-row">
			<a href="/">
				...
				if ($logo_url) {
					echo '<img class="w-auto h-6 sm:h-7" src="' . esc_url($logo_url) . '" alt="' . get_bloginfo('name') . '">';
				}
				?>
			</a>
			...
			<div class="flex mt-4 md:m-0">
				<div class="-mx-4">
					...
					foreach ($pages as $page) {
						...
						echo "<a href='{$page_link}' class='px-4 text-sm text-gray-600 transition-colors duration-300 dark:text-gray-200 hover:text-blue-500 dark:hover:text-blue-400 hover:underline'>{$page_title}</a>";
					}
					?>
				</div>
			</div>
